...
_HOST_PROC = ("\n\n**[호스트 AI 안내]** 교사의 현재 상황·단계에 맞춰 안내하세요. 관련 법 조항"
              "(교원지위법 등)이 있으면 **답변에 함께 밝히세요**. 확실치 않거나 범위 밖이면 "
...
_HOST_ROUTE = ("\n\n**[호스트 AI 안내]** 상황에 맞는 기관을 안내하되 애매하면 1395로 연결하세요. "
               "없는 기관·번호는 만들지 마세요.")
_HOST_DEFEND = ("\n\n**[호스트 AI 안내]** 위 방어 논리를 교사 상황에 맞춰 전하되, 근거 조항(제20조의6 등)"
                "·통계·연락처는 위 근거 그대로 **답변에 밝히고** 지어내지 마세요. "
                "정확한 조문 원문이 필요하면 `search_teacher_law`로 법제처 현행본을 확인해 제시하세요. "
                "소명서 등 서류가 필요해 보이면 '초안을 바로 써드릴게요'라고 제안하고 `generate_document`로 작성하세요.")
...
@mcp.tool(annotations={"title": "교권 침해 대응 절차 안내", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def guide_response_flow(current_stage: str = "", infringement_type: str = "") -> str:
    """교권119 서비스의 도구입니다. Guide a teacher through the 6-step response process after an
    ...
    out.append("**핵심 먼저**: ①침해행위 중단·현장 이탈 후 관리자(교감·교장)에 즉시 신고"
               "(폭행·성·협박이면 **즉시 112**) ②증거를 **육하원칙으로 당일** 확보(문자·녹음·목격자 진술) "
               "③**학교→교육지원청 24시간 내 보고**. 아래는 단계별 상세예요.")
    out.append("")
...
# 도구 2) 진술서·경위서 초안 생성
# ─────────────────────────────────────────────────────────────
@mcp.tool(annotations={"title": "진술서·경위서 초안 생성", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def draft_statement(
    who: str = "",
    when: str = "",
    where: str = "",
    ...
    """교권119 서비스의 도구입니다. Draft a Korean teacher's BASIC incident statement (경위서/진술서) for an
    educational-activity infringement (six-W factual narration with before/after
    context). Use this for a quick, generic 경위서.
    For a SPECIFIC official document — 사안 신고서 · 아동학대 소명서 · 교권보호위 의견서 ·
    목격자 진술서 · 즉시 분리 확인서 · 민원 답변/종결 · 특별휴가/치료비 신청서 — use
    `generate_document` (it matches the government/education-office standard forms).
    For the step-by-step PROCEDURE use `guide_response_flow`.
...
# ─────────────────────────────────────────────────────────────
@mcp.tool(annotations={"title": "아동학대 신고 대응 가이드", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def defend_child_abuse(situation: str = "") -> str:
    """교권119 서비스의 도구입니다. Guide a Korean teacher who is threatened with or reported for child abuse
    ...
    """
    out = [
        "**\"아동학대로 신고하겠다\"는 협박·신고에 대한 대응**",
        "",
        "**핵심 먼저**: 정당한 생활지도는 아동학대가 아니에요(초·중등교육법 **제20조의6**). "
        "신고돼도 **교육부 2024.7 집계상 약 85%가 교사에게 유리하게 종결**됐어요. "
        "지금 **1395**(평일 09~18·카톡 상시)로 신고·심리·법률·교육감 의견제출을 한 번에 연결하세요.",
        "",
        ...
        "",
        "**3. 든든한 근거 통계 (교육부 2024.7 집계 발표)**",
        "- 2023.9~2024.6 교육감 의견서 제출 553건 중 **약 70%가 '정당한 교육활동'** 입장(교육부 2024.7).",
        "- 정당하다고 본 사안: **불기소 57.3% + 수사개시 전 종결 28.2% ≈ 85%가 교사에게 유리하게 종결**(교육부 2024.7).",
        "",
        "**4. 지금 할 일 (구체적으로)**",
...
GUARD = ("\n\n위에 제시된 조문·시행일 등 근거만 사용하세요. 제시되지 않은 조항·숫자는 "
         "지어내지 말고 '미확인'으로 남기세요.")
...
def _cache_answer(law_name, article_no):
    ...
    head = (f"**{v.get('name', law_name)}** (시행 {e[:4]}-{e[4:6]}-{e[6:]}) — "
            "실시간 조회 실패로 **캐시본**을 보여드려요(최신본은 law.go.kr에서 확인).")
    arts = v.get("articles", {})
    if not article_no:
...
@mcp.tool(annotations={"title": "교권 법령·고시 조항 조회", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": True})
def search_teacher_law(law_name: str = "교원지위법", article_no: str = "") -> str:
    """교권119 서비스의 도구입니다. Look up a current Korean statute OR administrative-rule article relevant to
...
@mcp.tool(annotations={"title": "인용 법조항 진위 검증", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": True})
def verify_citation(law_name: str = "", article_no: str = "", claimed_content: str = "") -> str:
    """교권119 서비스의 도구입니다. Reverse-verify a law article that someone (a parent, complainant, etc.) cited
...
@mcp.tool(annotations={"title": "상황별 지원기관 연결", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def route_support(situation_type: str = "") -> str:
    """교권119 서비스의 도구입니다. Route a Korean teacher to the right support organizations for their
...
# 도구 6) 악성민원 응대 가이드
# ─────────────────────────────────────────────────────────────
@mcp.tool(annotations={"title": "악성민원 응대 가이드", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def guide_complaint_response(situation: str = "") -> str:
    """교권119 서비스의 도구입니다. Guide a Korean teacher on handling malicious complaints under the 2024
...
@mcp.tool(annotations={"title": "생활지도 상황별 근거 검색", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def guide_student_guidance(situation: str = "") -> str:
    """교권119 서비스의 도구입니다. For a described student-guidance situation, retrieve the most relevant
...
_ACTION_RULES = [
    (("성찰문", "반성문", "반성", "성찰"), 4,
     "성찰문(성찰하는 글쓰기)은 **「교원의 학생생활지도에 관한 고시」 제13조 훈계**의 과제예요.",
     ["전제: 조언·상담·주의·훈육을 거쳤는데도 개선이 없을 때만 가능(고시 제13조①).",
      "훈계 시 **사유와 개선방안을 함께 제시**해야 해요(제13조②).",
      "수업시간이 아닌 때에, 학칙 근거로.",
      "앞 단계 없이 바로 성찰문을 시키면 '정서학대' 근거로 쓰일 수 있어요."]),
    (("휴대폰", "스마트기기", "핸드폰", "폰", "기기"), 2,
     "수업 중 스마트기기 사용 제한은 **「학생생활지도 고시」 제11조 주의②**로 가능해요.",
     ["주의로 사용 제한 → 불응·위험 소지 의심 시 제12조 훈육④로 필요범위 내 보관.",
      "보관 절차: 주의 → 물품 보관 → 학교장 보고 → **보호자에게 인계**(학생에게 임의 처분 X).",
      "교과 수업시간 녹음은 개인정보·형사 위험. 증거는 원본 보존, 불리해도 삭제 금지."]),
    (("압수", "수거", "보관", "빼앗"), 3,
     "물품 보관·제한은 **「학생생활지도 고시」 제11조 주의 → 제12조 훈육④**(위험물품, 필요범위 내).",
     ["합리적 이유(위해 우려)가 있어야 하고, 보관 후 **학교장 보고 + 보호자 인계**.",
      "학생에게 임의로 처분·폐기하면 안 돼요."]),
    (("세워", "서있", "서 있", "손 들", "손들", "교실 뒤", "교실 앞", "기립", "벌 세"), 3,
     "교실 뒤·앞에 세우기, 손 들고 있기 같은 '벌 세우기'는 고시에 **명시된 훈육 방법이 아니에요.** "
     "짧고 비공개적이면 「학생생활지도 고시」 제12조 훈육 지시로 볼 여지가 있지만, 남용하면 학대 소지가 있어 신중해야 해요.",
     ["장시간 세워 두면 **신체적 고통 → 신체학대(아동복지법 제17조3호)** 소지가 있어요.",
      "반 앞에서 공개적으로 세워 창피를 주면 **정서학대(제17조5호)** 소지가 있어요.",
      "더 안전한 대안: **자리 이동, 별도 과업 부여, 잠깐 일어나 남은 과제 풀기** 등 '벌'이 아니라 '과제 완수'를 목적으로.",
      "꼭 해야 하면 **짧게·비공개적으로·건강 이상 없는 학생에 한해**, 목적과 시간을 기록으로 남기세요."]),
    (("청소", "원상복구"), 4,
     "청소·원상복구는 **「학생생활지도 고시」 제13조 훈계**의 과제예요(훼손 시설·물품 대상).",
     ["전제: 앞 단계(조언~훈육)를 거침 + 사유·개선방안 함께 제시."]),
    (("분리", "내보", "교실 밖", "제지"), 3,
     "수업 방해 학생 분리·제지는 **「학생생활지도 고시」 제12조 훈육**(+ 초·중등교육법 제20조의2 긴급 제지).",
     ["분리는 **교실 밖 '지정된 장소'에서 관리 하에** 해야 해요 — 복도에 그냥 세워두거나 방치하면 "
      "안전사고·**방임(아동학대) 소지**·학습권 침해가 돼요.",
      "분리 중에도 **학습권 보장**(교육자료·과제 제공). 긴급 제지는 위험 상황에서 최소한도로.",
...
@mcp.tool(annotations={"title": "지도단계 적법성 체커", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def check_guidance_legality(intended_action: str = "", steps_taken: str = "") -> str:
    """교권119 서비스의 도구입니다. Check whether a teacher's intended guidance measure follows the lawful
    ...
    # [1.5] 지금 이렇게 하세요 — 실전 방법을 사용자용 섹션으로 앞에(요약에도 생존)
    steps = _LADDER[:(idx + 1)] if idx is not None else _LADDER
    out.append("\n**[지금 이렇게 하세요 (단계별 실전)]**")
    out += [f"- **{st}** {_LADDER_HOWTO[st]}" for st in steps]
    ...
    out.append("① 생활지도 사다리(「교원의 학생생활지도에 관한 고시」): " + " → ".join(_LADDER) +
               " (낮은 단계부터 = 비례원칙, 건너뛰면 정당성↓·정서학대 소지).")
    ...
    out.append("**먼저** 이 조치가 ①사다리 몇 단계인지 ②학대 3-체크에 걸리는지 ③최소한도·학칙 근거인지를 "
    ...
    out.append("- 안내를 준 **뒤에**, 거친 단계나 조치 강도(얼마나 오래·공개적인지)가 안 적혀 있으면 "
               "'어디까지 하셨는지'를 한 줄 덧붙여 물어 답을 다듬으세요.")
...
@mcp.tool(annotations={"title": "학부모 대화 안전 도우미", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def safe_parent_message(situation: str = "", parent_message: str = "", teacher_draft: str = "") -> str:
    """교권119 서비스의 도구입니다. Help a teacher respond safely to a parent. Accepts ANY of: a free-text
    ...
    if ctx:
        ...
        if any(k in ctx for k in _PARENT_INTENT):
            hit = True
            out.append("- **의도 추궁** → '고의였냐'는 교사가 판단·인정할 문제가 아니에요. "
                       "\"의도는 제가 판단할 영역이 아니에요\"로 되돌리기.")
        if any(k in ctx for k in _PARENT_APOLOGY):
            hit = True
            out.append("- **사과 요구** → 사실 확인 전 잘못 인정 금지. 유감 표현과 사실 인정은 구분.")
        ...
        out.append("- **함께 점검할 유형**(해당되면 그 원칙 적용): "
...
@mcp.tool(annotations={"title": "마음 지지 (상담)", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def emotional_support(feeling: str = "") -> str:
    """교권119 서비스의 도구입니다. Give warm, counseling-style emotional support to a teacher in distress
...
@mcp.tool(annotations={"title": "진술서·경위서 검토", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def review_statement(document: str = "") -> str:
    """교권119 서비스의 도구입니다. Review a teacher's already-written incident statement (진술서/경위서/의견서):
    ...
    for name, kws in _REVIEW_ELEMENTS:
        ...
        out.append(f"- {'' if present else ''} {name}{'' if present else ' — 빠졌어요, 채우면 좋아요'}")
        if not present:
            missing.append(name)
...
# ─────────────────────────────────────────────────────────────
# 도구 13) 교권 침해 대응 문서 생성기 — 공식 서식 9종
# 근거: 2025 교육활동보호 매뉴얼 부록 서식(신고서1·분리확인서2·진술서4·의견서16-1) +
#       2026 학교민원 처리 매뉴얼(특이민원 지원요청·종결 문안) + 교육감 의견 제출 가이드라인(소명서)
...
_DOC_ALIASES = {
    ...
    "분리확인서": "분리확인서", "분리조치": "분리확인서", "분리": "분리확인서",
    "민원지원": "민원지원", "지원요청": "민원지원", "특이민원": "민원지원",
    "민원답변": "민원답변", "종결": "민원답변", "악성민원답변": "민원답변", "답변서": "민원답변",
    "특별휴가": "특별휴가", "휴가": "특별휴가",
    "치료비": "치료비", "심리상담": "치료비", "상담비": "치료비", "치료": "치료비",
}
_DOC_MENU = (
    "① **신고서** — 교육활동 침해 사안 신고서(교보위 절차 출발점)\n"
    "② **소명서** — 아동학대 신고 대응 소명서(정당한 생활지도 경위)\n"
    "③ **의견서** — 피해교원 서면 의견서(교권보호위 제출)\n"
    "④ **진술서** — 목격자·참고인 진술서\n"
    "⑤ **분리확인서** — 즉시 분리 의사 확인서\n"
    "⑥ **민원지원** — 특이민원(악성) 지원 요청 보고서\n"
    "⑦ **민원답변** — 반복·악성민원 답변/종결 통지\n"
    "⑧ **특별휴가** — 피해교원 특별휴가 신청서\n"
    "⑨ **치료비** — 심리상담·치료비 지원 신청서"
)
_DOC_TITLES = {
...
@mcp.tool(annotations={"title": "교권 침해 대응 문서 생성기(공식 서식 9종)", "readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False})
def generate_document(doc_type: str = "", who: str = "", when: str = "", where: str = "",
                      what: str = "", teacher: str = "", offender_type: str = "",
                      detail: str = "") -> str:
    """교권119 서비스의 도구입니다. Generate a Korean teacher's official document for an educational-activity
    infringement or a child-abuse-report defense, laid out to match the government /
    education-office STANDARD FORMS (2025 교육활동보호 매뉴얼 부록 · 2026 학교민원 처리 매뉴얼 ·
    ...
    Args:
        doc_type: Which document (e.g. "소명서", "신고서", "의견서", "진술서", "분리확인서",
                  "민원지원", "민원답변", "특별휴가", "치료비"). Empty → menu.
        who: The infringer / related person (침해 관련자 — 이름·소속).
        when: When it happened (일시).
        where: Where (장소).
        what: Factual account (경위, 육하원칙).
        teacher: The teacher (피해교원 — 성명·소속).
        offender_type: 학생 / 보호자 / 기타.
        detail: Document-specific detail — 소명서: 지도의 법령·학칙 근거·목적 / 민원: 처리 경위·이첩 사유 /
                치료비: 상담·치료 기관·비용 / 민원답변: 문제되는 표현.
    """
    def f(v, ph):
        return v.strip() if v.strip() else f"《{ph} — 채워 주세요》"

    def chk(v, t):
        return "☑" if v.strip() == t else "□"
    ...
    if not key:
        return ("어떤 문서를 만들까요? 상황을 말해 주시면 **맞는 문서를 추천하고 초안까지 바로 써드려요.**\n\n" + _DOC_MENU +
                "\n\n'아동학대로 신고당했는데 무슨 문서 써야 해?'처럼 상황만 말해도 돼요.\n"
    ...
    if key == "신고서":
        body = f"""교육활동 침해 사안 신고서 (초안)
========================================
[접수번호:            ] [접수일:      .   .   ]

1. 피해교원
   - 소속·성명: {f(teacher, '소속·성명')}   직위:            성별: □남 □여
   - 연락처:              교육경력:      년   구분: □정규 □기간제 / □담임 □특수교사
2. 침해 관련자
   - 구분: {ot}    소속·학년/반:
   - 성명: {f(who, '가해 관련자')}    성별: □남 □여   연락처:            특수교육대상: □예 □아니오
3. 보호자(관련자가 학생인 경우): 성명            연락처
4. 사안 내용 (육하원칙으로, 별지 사용 가능)
   {f(what, '누가·언제·어디서·무엇을·어떻게·왜 — 사실만')}
   (일시: {f(when,'일시')} / 장소: {f(where,'장소')})
5. 피해교원 요청사항: [ ] 즉시 분리 희망(→ 분리확인서 첨부) [ ] 희망하지 않음
6. 목격자(참고인): □무 □유(성명·관계:                    )
7. 분쟁조정: [ ] 신청함 [ ] 신청하지 않음
8. 입증자료: (녹음·문자캡처·CCTV·진단서·목격자 진술서 등)

위와 같이 신고합니다.     20   .    .    .      신고인:                (서명 또는 인)
수신: ○○학교장 귀중 / ○○교육지원청교육장 귀중"""
        tips = ("\n**팁**: 4번 사안 내용은 침해유형(공무방해·모욕·협박 등 16종) 판정 근거예요 — "
                "언행을 **들은 그대로 인용**하고 전후 맥락을 적으세요.")

    elif key == "소명서":
        body = f"""아동학대 신고 대응 소명서 (정당한 생활지도 소명) (초안)
========================================
※ 교육감 의견서(교육청 작성)의 근거자료로 제출하는 문서입니다.

1. 교원: {f(teacher,'소속·성명·담당')}
2. 신고된 행위 요지 / 일시·장소: {f(what,'신고 내용 요지')} / {f(when,'일시')} {f(where,'장소')}
3. 당시 생활지도 경위 (육하원칙): {f(what,'무엇을 지도하던 중 어떤 조치를 했는지 — 사실')}
4. 지도의 법령·학칙 근거: {f(detail,'초·중등교육법 제20조의2, 학생생활지도 고시, 학칙·학급규칙 중 해당 근거')}
5. 교육적 목적·필요성: 《이 지도가 학생 행동 개선·안전·다른 학생 학습권을 위해 필요했음 — 채워 주세요》
6. 고의·중대과실 없음 소명: 《수단이 통상적·최소한·비례적이었고 감정적·징벌적이 아니었음 — 채워 주세요》
7. 증거: (CCTV 열람·문자·동료 진술 등)

작성일: 20   .    .    .        작성자(교원):                (서명)

■ 근거 법령: 「초·중등교육법」 제20조의2(생활지도) · 「교원의 학생생활지도에 관한 고시」 ·
   제20조의6(정당한 생활지도는 「아동복지법」 금지행위로 보지 않음)"""
        tips = ("\n**정당한 생활지도 3요소로 채우세요(소명의 핵심)**\n"
                "① **법령·학칙 근거** — 어떤 규정에 따라 지도했는가 (4번)\n"
                "② **교육적 목적** — 왜 그 지도가 필요·정당했는가 (5번)\n"
                "③ **고의·중대과실 없음** — 수단이 통상적·비례적이었음 (6번)\n"
                "→ 세 요소가 서면 '정당한 생활지도'로 인정돼 **제20조의6 면책**으로 이어져요. (CCTV로 사실 부존재 입증 사례 다수)")

    elif key == "의견서":
        body = f"""피해교원 의견서 (교권보호위원회 서면 제출) (초안)
========================================
1. 피해교원: {f(teacher,'소속·성명·직위')}
2. 침해 관련자: {f(who,'성명')}   구분: {ot}
3. 의견 내용
   위 교육활동 침해 신고 사안에 대해 아래와 같이 의견을 제출합니다.
   (1) 사안 발생 당시 상황: {f(what,'무슨 일이 있었는지')}
   (2) 피해 상황(구체적): 《받은 피해 — 신체·정신·수업 지장 등 — 채워 주세요》
   (3) 관련자 조치·본인 보호조치 의견: 《엄정 조치 / 관계회복 희망 / 필요한 보호조치 — 채워 주세요》
4. (심의 불참 시) 사유:
작성일: 20   .   .   .    작성자:            (서명)
수신: ○○교육지원청 지역교권보호위원회 위원장 귀중"""
        tips = "\n**팁**: (3)의 조치 의견은 위원회의 '반성·관계회복 정도' 판단에 반영돼요 — 원하는 조치를 분명히 적으세요."

    elif key == "진술서":
        body = f"""목격자(참고인) 진술서 (초안)
========================================
1. 참고인: {f(who,'성명')}   소속/학년·반:            연락처:
2. 피해교원 또는 침해 관련자와의 관계:
3. 목격한 사실 (본 것·들은 것만, 육하원칙)
   {f(what,'언제·어디서·누가·무엇을 하는 것을 보았는지 — 추측·평가 배제')}
   (일시: {f(when,'일시')} / 장소: {f(where,'장소')})
4. 입증자료 첨부:
작성일: 20   .   .   .    작성자(참고인):            (서명)"""
        tips = "\n**팁**: 목격자가 여러 명이면 각자 개별 진술서를 써야 해요. '본 것/들은 것'만 — 의견·추측은 빼세요."

    elif key == "분리확인서":
        body = f"""즉시 분리 의사 확인서 (초안)
========================================
1. 분리 희망 여부
   「교원의 지위 향상 및 교육활동 보호를 위한 특별법」 제20조제2항에 따른
   가해 학생과의 즉시 분리를  [ ] 희망합니다   [ ] 희망하지 않습니다
2. 안내
   - 분리는 7일 범위에서 시행하며, 별도 공간에서 학생의 학습권을 보장합니다.
   - 피해교원의 반대 의사 등 특별한 사유가 없으면 즉시 분리가 원칙입니다.
피해교원: {f(teacher,'소속·성명')}   관련 학생: {f(who,'성명·학년반')}
작성일: 20   .   .   .    신고인:            (서명 또는 인)
수신: ○○학교장 귀중"""
        tips = "\n**팁**: 분리 시행 결과는 학교의 '발생보고서 분리조치란'에 기록돼요. 분리를 원하면 1번에 체크하세요."

    elif key == "민원지원":
        body = f"""특이민원(교육활동 침해) 지원 요청 보고서 (초안)
========================================
1. 학교명: {f(teacher,'학교명')}
2. 제기 대상(교원):
3. 분류: □교무/학사 □생활지도/상담 □학부모·교직원 소통 □학교 운영 □기타
4. 민원 관련 학생: {f(who,'성명·학급')}
5. 보호자: 성명            연락처
6. 민원 제기 및 처리 내용 (일자별 경위)
   {f(what,'언제 어떤 민원이 있었고 학교가 어떻게 답변·처리했는지 — 일자별')}
7. 이첩 사유(학교 자체 해결 노력 포함)
   {f(detail,'예: 합리적 답변에도 3회 이상 동일·유사 민원을 반복 제기·위력 행사')}
8. 교육(지원)청 요청 사항:
작성일: 20   .   .   .    보고자:            (서명)"""
        tips = "\n**팁**: 7번 이첩 사유엔 '학교가 이미 이만큼 응대했다'를 일자별로 남기세요 — 반복·악성 판단의 근거예요."

    elif key == "민원답변":
        body = f"""민원 답변 / 반복·악성민원 종결 통지 (공문 초안)
========================================
수 신: {f(who,'민원인')}
제 목: 귀하의 민원({f(what,'민원 제목·요지')})에 대한 회신

1. 귀하께서 제기하신 민원에 대해 아래와 같이 답변드립니다.
   - 민원 요지: {f(what,'민원 내용')}
   - 처리(답변) 결과: 《검토 결과·조치 내용 — 채워 주세요》
2. (반복민원인 경우) 「민원 처리에 관한 법률」 제23조에 따라, 동일한 내용의 민원을
   정당한 사유 없이 3회 이상 반복 제기하시어, 2회 이상 처리결과를 통지한 이후
   접수되는 동일 민원은 종결 처리됨을 알려드립니다.
3. (경고, 해당 시) 민원 내용 중 '{f(detail,'문제되는 표현')}'은 관련 법령에 저촉될 수 있으며,
   반복 시 학교는 법적 조치 등을 취할 수 있음을 알려드립니다.

20   .   .   .        ○○학교장 (직인)"""
        tips = "\n**팁**: 민원 대응은 개인이 아니라 **학교 민원대응팀** 명의로. 종결·경고는 법 제23조 근거를 밝혀 정중히."

    elif key == "특별휴가":
        body = f"""특별휴가 신청서 (교육활동 침해 피해교원) (초안)
========================================
1. 신청인: {f(teacher,'소속·직위·성명')}
2. 휴가 종류: 특별휴가 (교육활동 침해 피해 회복)
3. 휴가 기간: 20  .  .  ~ 20  .  .   (총    일)   ※ 5일 범위(중대범죄 시 +5일)
4. 사유: {f(what,'교권 침해 사안 개요 — 간단히')}
5. 첨부: 침해 사안 신고서·발생보고서 등 사유 입증 자료
신청일: 20   .   .   .    신청인:            (서명)
[학교장 결재]"""
        tips = "\n**근거**: 교원지위법 제23조 + 「교원휴가에 관한 예규」 — 피해교원은 5일 범위 특별휴가를 쓸 수 있어요(폭행·성폭력 등은 추가)."

    else:  # 치료비
        body = f"""심리상담·치료비 지원 신청서 (초안)
========================================
1. 신청인(피해교원): {f(teacher,'소속·성명')}   연락처:
2. 침해 사안 개요: {f(what,'일시·행위 요지')}   ({f(when,'일시')})   교보위 처분: □유 □무
3. 상담·치료 내역: {f(detail,'상담·치료 기관명, 내역, 기간')}
4. 청구 비용:            원   지급 계좌:
5. 첨부: 진단서·소견서, 상담·치료 영수증, 사안 신고서/발생보고서, 교보위 조치 의결서 등
신청일: 20   .   .   .    신청인:            (서명)
제출: 소속 시·도 학교안전공제회(교육활동보호센터 안내)"""
